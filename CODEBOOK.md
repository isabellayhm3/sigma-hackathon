...
| `control_chart.R` | Generates an X bar control chart | ggplot |
| `calculate_ttf.R` | Calculates Time to Failure (TTF) for each solar panel | List containing `ttf_results` and `mean_ttf` |
| `plot_ttf.R` | plots top N panels at risk of failure using horizontal bar charts with color-coded risk categories (high, moderate, low). | `ggplot` bar chart |
